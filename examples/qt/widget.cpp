...
Widget::Widget(QWidget* parent) : QWidget(parent), client_(ioc_, this) {
...
void Widget::on_frame_update(const libvnc::frame_buffer& buffer) {
   image_ = QImage(buffer.data(), buffer.width(), buffer.height(), QImage::Format_RGB32);
   this->update();
}
...
void Widget::on_cursor_shape(int xhot,
   ...
   auto image = QImage(rc_source.data(), rc_source.width(), rc_source.height(), QImage::Format_ARGB32);
