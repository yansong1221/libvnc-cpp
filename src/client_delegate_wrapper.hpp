...
public:
    ...
    {
        return invoke(&client_delegate::on_new_frame_size, w, h);
    }
    auto on_keyboard_led_state(int state)
    {
        return invoke(&client_delegate::on_keyboard_led_state, state);
    }
    auto on_frame_update(const frame_buffer& frame)
    {
        return invoke(&client_delegate::on_frame_update, frame);
    }
    ...
    auto on_cut_text_utf8(std::string_view text)
    {
        return invoke(&client_delegate::on_cut_text_utf8, text);
    }
