...
public:
    ...
    void set_client2server(int type, bool val = true) { client2server_.set(type, val); }
    void set_server2client(int type, bool val = true) { server2client_.set(type, val); }

    bool supports_client2server(int type) { return client2server_.test(type); }
    bool supports_server2client(int type) { return server2client_.test(type); }
    ...
    void print()
    {
        spdlog::info("client2server supported messages (bit flags): {}",
                     client2server_.to_string());
        spdlog::info("server2client supported messages (bit flags): {}",
                     server2client_.to_string());
    }
