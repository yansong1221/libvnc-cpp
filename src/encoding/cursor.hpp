...
public:
					     ...
					     frame_buffer &buffer, std::shared_ptr<frame_op> op) override
	{
		...
		if (auto err = co_await read_rc_source(socket, width, height, bytesMaskData, bytesPerRow); err)
			co_return err;
...
public:
						     ...
						     std::size_t bytesMaskData, std::size_t bytesPerRow) override
	{
		...
		co_await boost::asio::async_read(socket, boost::asio::buffer(&rgb, sizeof(rgb)), net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);

		colors[0] = rgb24_to_pixel(rcSource_.pixel_format(), rgb.backRed.value(), rgb.backGreen.value(),
					   rgb.backBlue.value());
		colors[1] = rgb24_to_pixel(rcSource_.pixel_format(), rgb.foreRed.value(), rgb.foreGreen.value(),
					   rgb.foreBlue.value());
...
private:
	inline static uint32_t rgb24_to_pixel(const proto::rfbPixelFormat &format, uint8_t r, uint8_t g, uint8_t b)
	{
		return ((((uint32_t)(r) & 0xFF) * format.redMax.value() + 127) / 255 << format.redShift.value() |
			(((uint32_t)(g) & 0xFF) * format.greenMax.value() + 127) / 255 << format.greenShift.value() |
			(((uint32_t)(b) & 0xFF) * format.blueMax.value() + 127) / 255 << format.blueShift.value());
	}
};
...
public:
	...
	{
		boost::system::error_code ec;
		co_await boost::asio::async_read(socket, boost::asio::buffer(rcSource_.data(), rcSource_.size()),
						 net_awaitable[ec]);
		co_return error::make_error(ec);
	}
};
