...
public:
	...
	void init() override {}
	proto::rfbEncoding encoding_code() const override { return proto::rfbEncoding::rfbEncodingExtDesktopSize; }
					     ...
					     frame_buffer &buffer, std::shared_ptr<client_op> op) override
	{
		...
		co_await boost::asio::async_read(socket, boost::asio::buffer(&eds, sizeof(eds)), net_awaitable[ec]);
		if (ec)
		...
		for (int loop = 0; loop < screen_num; loop++) {
			co_await boost::asio::async_read(socket, boost::asio::buffer(&screen, sizeof(screen)),
							 net_awaitable[ec]);
			if (ec)
				co_return error::make_error(ec);
