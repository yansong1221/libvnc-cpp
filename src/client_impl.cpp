...
boost::asio::awaitable<libvnc::error> client_impl::async_handshake()
{
	...
	co_await boost::asio::async_read(*stream_, boost::asio::buffer(&pv, sizeof(pv)),
	...
	co_await boost::asio::async_write(*stream_, boost::asio::buffer(&pv, proto::sz_rfbProtocolVersionMsg),
					  net_awaitable[ec]);
...
boost::asio::awaitable<error> client_impl::async_authenticate()
{
	...
	if (major_ == 3 && minor_ > 6) {
		...
		uint8_t count = 0;
		co_await boost::asio::async_read(*stream_, boost::asio::buffer(&count, sizeof(count)),
						 net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);
		...
		co_await boost::asio::async_write(*stream_, boost::asio::buffer(&selected_auth_scheme, 1),
						  net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);
	} else {
		...
		co_await boost::asio::async_read(*stream_, boost::asio::buffer(&authScheme, sizeof(authScheme)),
						 net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);
...
boost::asio::awaitable<error> client_impl::server_message_loop()
{
	...
	for (;;) {
		...
		co_await boost::asio::async_read(*stream_, boost::asio::buffer(&msg_id, sizeof(msg_id)),
						 net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);
...
boost::asio::awaitable<error> client_impl::read_auth_result()
{
	...
	co_await boost::asio::async_read(*stream_, boost::asio::buffer(&authResult, sizeof(authResult)),
					 net_awaitable[ec]);
	if (ec)
		co_return error::make_error(ec);
...
boost::asio::awaitable<error> client_impl::read_error_reason()
{
	...
	co_await boost::asio::async_read(*stream_, boost::asio::buffer(&reasonLen, sizeof(reasonLen)),
					 net_awaitable[ec]);
	if (ec)
		co_return error::make_error(ec);
...
boost::asio::awaitable<libvnc::error> client_impl::AuthRSAAES(int keySize, bool encrypted)
{
	...
	try {
		...
		if (u32_length.value() < MinRsaKeyLength) {
			co_return error::make_error(custom_error::auth_error,
						    fmt::format("Server RSA key is too small ({})",
								u32_length.value()));
		}
		...
		Botan::RSA_PublicKey _server_rsa_pub_key(Botan::BigInt::from_bytes(modulus),
							 Botan::BigInt::from_bytes(exp));
		...
		Botan::PK_Encryptor_EME enc(_server_rsa_pub_key, rng, "EME-PKCS1-v1_5");

		auto buffer = enc.encrypt(client_random_key.bits_of(), rng);
		...
		Botan::PK_Decryptor_EME dec(_client_rsa_key, rng, "EME-PKCS1-v1_5");
		Botan::SymmetricKey server_random_key(dec.decrypt(buffer));
		...
		if (keySize == 128)
			...
			co_return error::make_error(custom_error::auth_error,
						    fmt::format("unknown AES bit({})", keySize));
...
boost::asio::awaitable<libvnc::error> client_impl::on_rfbFramebufferUpdate()
{
	...
	for (int i = 0; i < msg.num_rects.value(); ++i) {
		co_await boost::asio::async_read(*stream_, boost::asio::buffer(&UpdateRect, sizeof(UpdateRect)),
						 net_awaitable[ec]);
		if (ec)
			co_return error::make_error(ec);
